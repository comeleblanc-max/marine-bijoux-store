      ...
      {crumbs.map((crumb, i) => (
        <span key={i} className="flex items-center gap-1">
          ...
          {crumb.href && i < crumbs.length - 1 ? (
            <Link href={crumb.href} className="hover:text-[#24BBD0] transition-colors">{crumb.label}</Link>
          ) : (
            <span className={i === crumbs.length - 1 ? 'text-[#24BBD0] font-medium' : ''}>{crumb.label}</span>
          )}
        </span>
      ))}
    </nav>
