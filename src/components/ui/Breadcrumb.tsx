      ...
      {crumbs.map((crumb, i) => (
        <span key={i} className="flex items-center gap-1">
          ...
          {crumb.href && i < crumbs.length - 1 ? (
            <Link href={crumb.href} className="hover:text-[#1A3A52] transition-colors">{crumb.label}</Link>
          ) : (
            <span className={i === crumbs.length - 1 ? 'text-[#1A3A52] font-medium' : ''}>{crumb.label}</span>
          )}
        </span>
      ))}
    </nav>
