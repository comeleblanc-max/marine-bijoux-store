    ...
    <Link href={`/products/${product.slug}`} className="group block">
      ...
      <div className="relative aspect-square bg-[#FAF5EA] overflow-hidden mb-4">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          {product.newArrival && (
            <span className="bg-white/95 text-[#1F3A56] text-[9px] tracking-[0.2em] uppercase font-medium px-2.5 py-1">
              Nouveau
            </span>
          )}
          {discount && (
            <span className="bg-[#1F3A56] text-white text-[9px] tracking-[0.2em] uppercase font-medium px-2.5 py-1">
              -{discount}%
            </span>
          )}
          {!product.inStock && (
        ...
        >
          <Heart
            className={`w-[14px] h-[14px] transition-colors ${
              wished ? 'fill-[#D4AF37] text-[#D4AF37]' : 'text-[#1F3A56]'
            }`}
            strokeWidth={1.5}
          />
        </button>
        ...
        {product.inStock && (
          <button
            onClick={onAdd}
            className="absolute bottom-0 left-0 right-0 bg-[#1F3A56] text-white py-3 text-[10px] tracking-[0.25em] uppercase font-medium flex items-center justify-center gap-2 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out"
          >
      ...
      <div className="space-y-1.5">
        ...
        <h3 className="text-sm text-[#1F3A56] group-hover:text-[#D4AF37] transition-colors leading-snug">
          {product.name}
        </h3>
        <div className="flex items-center gap-2">
          <span className="text-sm text-[#1F3A56] font-medium">
