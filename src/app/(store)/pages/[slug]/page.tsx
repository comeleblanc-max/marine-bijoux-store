  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <article className="max-w-3xl mx-auto px-4 py-16">
          ...
          {page.sections.map((section, i) => (
            <Reveal key={i} delay={i * 0.04}>
              <section>
                {section.heading && (
                  <h2
                    className="text-xl sm:text-2xl text-[#1A3A52] font-medium mb-4"
                    style={{ fontFamily: 'var(--font-playfair)' }}
                  >
                    {section.heading}
                  </h2>
        ...
        {/* Aide */}
        <Reveal delay={0.1}>
          <div className="mt-16 p-6 bg-[#F5F1ED] rounded-2xl text-center">
            <p className="text-[#1A3A52] font-medium mb-1">Une question ?</p>
            <p className="text-gray-500 text-sm mb-4">
              Notre équipe vous répond avec plaisir sous 24 h.
            </p>
            <Link
              href="/pages/contact"
              className="inline-flex items-center gap-2 bg-[#C9A84C] hover:bg-[#b8963e] text-white px-6 py-3 rounded-full font-medium transition-colors"
            >
              Nous contacter
            </Link>
          </div>
